...
- **启动等待态**：每次完整加载显示品牌 Logo 与启动阶段，初始化完成前隐藏工作台。浏览器与 Linux 使用纯主题底色；darwin/win32 使用 28% `surface` 染色，透出原生材质。所有平台均隐藏 Session 内容。退出时 Logo 缩小淡出，遮罩与工作台交叉淡变。`prefers-reduced-transparency` 回退不透明 `surface`。
