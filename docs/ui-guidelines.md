...
- **启动等待态**：每次完整加载逐帧播放 Pi 组装动画，落块关键帧之间平滑补间，消行与闪烁保持离散。初始化完成前隐藏工作台。背景用 92% `surface` 染色；darwin/win32 透出原生材质，浏览器与 Linux 使用主题底色，不透出 Session 内容。`prefers-reduced-transparency` 回退不透明 `surface`。
