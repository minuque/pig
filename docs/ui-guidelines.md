...
- **启动等待态**：每次完整加载逐帧播放 Pi 组装动画，落块关键帧之间平滑补间，消行与闪烁保持离散。背景用 92% `surface` 染色；darwin/win32 透出原生材质，浏览器与 Linux 用 `backdrop-filter` 模拟。`prefers-reduced-transparency` 回退不透明 `surface`。
