...
- 每次完整加载逐帧播放 Pi 组装动画。落块关键帧之间平滑补间，消行与闪烁保持离散。遮罩只播 Logo，不展示验证凭证 / 连接 Pi / 准备工作台文案。
- 动画与 boot 并行。Logo 播完即离场进工作台，不等网关连上。
- 各平台用不透明 `surface`。退出时 Logo 缩小淡出，遮罩与工作台交叉淡变。
- bootstrap / connect / initialize 失败进 `/error`（含连接超时）。已连接后的连接错误同一块 `StartupError` 空画布：icon + 文本，水平垂直居中。侧栏和顶栏仍在。
...
- 正常时无常驻指示。连接中不挡欢迎/对话页，只在顶栏给一句状态。错误走主栏空画布，与启动失败同款。
