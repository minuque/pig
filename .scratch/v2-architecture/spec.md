# no-pi-no-gang v2 阶段一规格

Status: target-architecture

本文件是阶段一目标架构的唯一规范入口，用于生成和评审实现 Issues。Issue 记录实现切片与当前状态；研究材料和旧规格仅提供历史证据。现有架构图是非规范历史视图，不得覆盖本文件中的决策。
...
阶段一交付一个通过 npm 安装的本地单用户公开 Beta：Vue 客户端通过版本化 REST 与 Gateway 级 SSE 连接一个本地 Node Agent Gateway，Gateway 嵌入项目固定的 Pi 版本。

发布范围：

- 支持 Windows 和 Linux；macOS 不作阶段一发布承诺。
...
- 阶段一固定 Pi 版本，只支持应用 SQLite 的前向迁移，不承诺二进制降级或跨 Pi 版本回滚。
...
- Gateway 校验本地 authority、请求来源、Cookie、CSRF 和 Workspace Grant。
...
- Session、Run 和查询只能通过有效 Workspace Grant 访问。
- 取消 Grant 时不得让仍在执行的 Run 失去归属。
- 产品必须明确说明 Workspace Grant 不是操作系统沙箱，不限制 Pi 工具或子进程的文件权限。

### 2.3 Durable Session workbench

- 用户可以发现、创建、恢复、分页、重命名、搜索和删除已授权 Workspace 中的 Pi Session。
- 用户可以安全查看 Transcript、Markdown、代码、思考和工具活动，并浏览较早历史。
...
- SSE 断线、Gateway epoch 变化或客户端无法证明连续性时，客户端重新获取 snapshot；阶段一不承诺补发断线期间的每个增量事件。
...
- 诊断输出不得持久化 prompt、Transcript、工具 payload、credential、Cookie、token、环境值或原始路径。
...
- Pi Session JSONL：Session、Transcript、工具结果、模型变化、压缩及 Pi 扩展内容的唯一事实源。
- Application SQLite：Principal、Workspace Grant、Run 账本、命令幂等、资源 revision，以及可从 Pi JSONL 重建的列表和搜索索引。
...
3. Workspace 必须显式授权，且 Workspace Grant 不得被描述为 sandbox。
4. Pi JSONL 是 Session 与 Transcript 的唯一事实源；SQLite 不是 Message Store。
5. 应用数据库和日志不得保存 credential、完整 prompt、Transcript 或工具 payload。
6. 每个 Session 同时最多一个 active Run；同 Session FIFO，不同 Session 有界并行。
7. Execution Profile 在 Run admission 时冻结。
8. 每个 mutation 都有客户端生成的命令身份；等价重试返回原结果，不同 payload 复用身份必须拒绝。
...
16. 发布包固定 Pi 版本；阶段一不在普通更新中改变该版本。
...
12. 核心 Workspace、Session 和 conversation 旅程可通过键盘完成，并在窄屏与浅/深主题下可用。
...
- Transcript 虚拟化、raw HTML、专用文件 diff/图片展示和未验证的性能优化。
