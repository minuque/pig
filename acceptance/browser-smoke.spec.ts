...
test("packed Gateway bootstrap, SSE, and composer smoke", async ({ page, gateway }) => {
  ...
  await expect(page.getByText("Gateway access only; not a filesystem sandbox")).toBeVisible();
  ...
  await page.getByLabel("输入 Prompt").fill("stream the release answer");
  const send = page.getByRole("button", { name: "发送" });
  await expect(send).toBeEnabled();
  await send.click();
  const liveOutput = page.getByRole("article", { name: "进行中的输出" });
  await expect(liveOutput).toContainText("Release");
  const durableOutput = page.getByRole("article", { name: "助手消息" });
  await expect(durableOutput).toContainText("Release stream complete.");
